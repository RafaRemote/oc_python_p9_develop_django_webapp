...
def loginPage(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            message1 = "😕 cet utilisateur n'existe pas"
            message2 = "🤔 ou votre mot de passe est faux ?"
...
def abo(request):
    ...
    if request.method == 'POST':
        ...
        if followed not in followeds:
            new = UserFollow.objects.create(user=actualuser,
...
def create_ticket(request):
    ...
    if request.method == "POST":
        ticket = Ticket.objects.create(titre=request.POST.get('titre'),
...
def create_critique(request):
    ...
    if request.method == 'POST' and request.POST.get('type_produit') is not None:
        ...
        actualuser = User.objects.get(username=request.user)
        review = Review.objects.create(type_produit=request.POST.get('type_produit'),
...
def answer_ticket(request, pk):
    ...
    if request.method == 'POST':
        review = Review.objects.create(ticket=tick,
